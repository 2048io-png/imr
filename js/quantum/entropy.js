const ENTROPY = {
  ...
  gain() {
    ...
    if (tmp.en.eff.hr.eq(0)) return E(0);
    return x;
  },
  cap() {
    let x = tmp.en.eff.hr;
  ...
  rewards: [
    {
      ...
      eff(i) {
        let x = hasElement(214) ? Decimal.pow(1.1, i.pow(0.8)) : hasElement(114) ? i.add(1).root(1.5) : i.div(2).add(1).root(3);
        return x;
      },
      desc(x) {
    ...
    },
    {
      ...
      desc(x) {
        return `Atomic Power’s effect is <b>${formatPercent(x.sub(1))}</b> exponentially stronger.`.corrupt(tmp.c16active);
      },
    },
    {
    ...
    },
    {
      ...
      desc(x) {
        return `Tickspeed Power gives <b>^${x[0].format(2)}</b> boost to BHC & Cosmic Ray Powers.<br>Currently: <b>${x[1].format()}x</b>`.corrupt(tmp.c16active);
      },
    },
    {
      ...
      desc(x) {
        return `<b>x${x.format(2)}</b> extra Mass upgrades, Tickspeed, BHC and Cosmic Ray.`.corrupt(tmp.c16active);
      },
    },
    {
    ...
    },
    {
      ...
      desc(x) {
        return `Radiation effects are boosted by <b>^${x.format()}</b> based on Entropy.`.corrupt(tmp.c16active);
      },
    },
    {
      title: "Entropic Booster",
      ...
      eff(i) {
        let x = E(1e20).log(1e15).mul(i).softcap(10, 0.5, 0);
        if (x.lte(0)) return E(1);
        else return x;
      },
      desc(x) {
        return `Entropy boosts Pre-Quantum Global Speed by <b>${x.format()}x</b>.`;
      },
    },
    /*
        {
...
function updateEntropyHTML() {
  ...
  tmp.el.evaBtn1.setHTML(player.qu.en.eth[0] ? `Stop Evaporating to get<br>${player.qu.en.eth[1].format()}<br>best Enthalpy` : `Evaporate your frequency to gain Enthalpy`);
  tmp.el.evaBtn2.setHTML(player.qu.en.hr[0] ? `Stop Evaporating to get<br>${player.qu.en.hr[1].format()}<br>best Hawking Radiation` : `Evaporate your mass of Black Hole to gain Hawking Radiation`);
