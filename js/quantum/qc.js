const QCs = {
  active() {
    return player.qu.qc.active || player.qu.rip.active || CHALS.inChal(14) || CHALS.inChal(15) || tmp.c16active || inDarkRun();
  },
  getMod(x) {
...
function updateQCHTML() {
  tmp.el.qc_shard.setTxt(
    player.qu.qc.shard +
      (tmp.qu.qc_s + tmp.qu.qc_s_bouns != player.qu.qc.shard ? ` (${tmp.qu.qc_s + tmp.qu.qc_s_bouns >= player.qu.qc.shard ? "+" : ""}${tmp.qu.qc_s + tmp.qu.qc_s_bouns - player.qu.qc.shard})` : "")
  );
