const MASS_DILATION = {
  ...
  massGain() {
    ...
    if (player.ranks.tier.gte(70)) x = x.mul(RANKS.effect.tier[70]());
    if (player.ranks.rank.gte(1400)) x = x.mul(RANKS.effect.rank[1400]());
  ...
  effect() {
    let x = tmp.md.bd3
      ? player.qu.rip.active
        ? player.md.mass.max(1).log10().max(1).log10().add(1).log10().add(1).root(2)
    ...
    ids: [
      {
        ...
        bulk() {
          return player.md.mass.gte(10) ? player.md.mass.div(10).max(1).log10().add(1).floor() : E(0);
        },
        effect(x) {
          let b = 2;
          if (hasElement(25)) b++;
          return E(b)
            ...
            .softcap("e1.2e4", 0.96, 2); //.softcap('e2e4',0.92,2)
        },
        effDesc(x) {
          return format(x, 0) + "x" + (x.gte("e1.2e4") ? ` <span class='soft'>(softcapped${x.gte("e2e400") ? "^2" : ""})</span>` : "");
        },
      },
      {
        desc: `Make dilated mass effect stronger.`,
        cost(x) {
          return tmp.md.bd3 ? E(10).pow(E(1.25).pow(x)).mul(100) : E(10).pow(x).mul(100);
        },
        bulk() {
          return player.md.mass.gte(100) ? (tmp.md.bd3 ? player.md.mass.div(100).max(1).log10().max(1).log(1.25).add(1).floor() : player.md.mass.div(100).max(1).log10().add(1).floor()) : E(0);
        },
        effect(x) {
          if (tmp.md.bd3)
            return x
              .mul(tmp.md.upgs[11].eff || 1)
              .root(player.qu.rip.active || tmp.c16active || player.dark.run.active ? 3 : 2)
              .mul(player.qu.rip.active || tmp.c16active || player.dark.run.active ? 0.05 : 0.1)
              .add(1);
        ...
        effDesc(x) {
          return (x.gte(10) ? format(x) + "x" : format(x.sub(1).mul(100)) + "%") + " stronger";
        },
      },
      {
        ...
        effDesc(x) {
          return format(x, 0) + "x" + (x.gte(1e25) ? " <span class='soft'>(softcapped)</span>" : "");
        },
      },
      {
        desc: `Dilated mass also boost Stronger's power and mass gain.`,
        maxLvl: 1,
        cost(x) {
      ...
      },
      {
        ...
        bulk() {
          return player.md.mass.gte(E(1.619e20).mul(1e4)) ? player.md.mass.div(E(1.619e20).mul(1e4)).max(1).log(1e5).add(1).floor() : E(0);
        },
        effect(x) {
      ...
      },
      {
        ...
        bulk() {
          return player.md.mass.gte(1.5e73) ? player.md.mass.div(1.5e73).max(1).log(1e3).max(0).root(1.5).add(1).floor() : E(0);
        },
        effect(i) {
        ...
        effDesc(x) {
          return "+^" + format(x) + (x.gte(1e3) ? " <span class='soft'>(softcapped)</span>" : "");
        },
      },
      {
      ...
      },
      {
        ...
        effect(x) {
          return player.tickspeed.add(1).pow(2 / 3);
        },
        effDesc(x) {
          return format(x) + "x";
        },
      },
      {
        ...
        bulk() {
          return player.md.mass.gte("1.50001e536") ? player.md.mass.div("1.50001e536").max(1).log(5).add(1).floor() : E(0);
        },
        effect(x) {
          return E(2)
            .pow(x)
        ...
        effDesc(x) {
          return format(x) + "x" + (x.gte(1e25) ? " <span class='soft'>(softcapped)</span>" : "");
        },
      },
      {
        unl() {
        ...
        bulk() {
          return player.md.mass.gte("1.50001e1556") ? player.md.mass.div("1.50001e1556").max(1).log(1e50).max(0).root(1.5).add(1).floor() : E(0);
        },
        effect(i) {
      ...
      ids: [
        {
          ...
          bulk() {
            return player.md.break.mass.gte(1e5) ? player.md.break.mass.div(1e5).max(1).log10().root(1.1).add(1).floor() : E(0);
          },
          effect(y) {
        ...
        },
        {
          ...
          bulk() {
            return player.md.break.mass.gte(1e7) ? player.md.break.mass.div(1e7).max(1).log10().root(1.25).add(1).floor() : E(0);
          },
          effect(y) {
            let x = y.div(40);
        ...
        },
        {
          ...
          bulk() {
            return player.md.break.mass.gte(1.989e36) ? player.md.break.mass.div(1.989e36).max(1).log10().root(2).add(1).floor() : E(0);
          },
          effect(y) {
        ...
        },
        {
          ...
          bulk() {
            return player.md.break.mass.gte(2.9835e48) ? player.md.break.mass.div(2.9835e48).max(1).log10().root(1.5).add(1).floor() : E(0);
          },
          effect(y) {
        ...
        },
        {
          ...
          effDesc(x) {
            return x[0].format() + "x to Entropies gain, " + x[1].format() + "x to Death Shards gain";
          },
        },
        {
