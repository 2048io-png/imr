const UPGS = {
  mass: {
    ...
    getData(i) {
      ...
      if (i == 4) {
        ...
        if (i == 1 && player.ranks.rank.gte(8)) inc = inc.pow(E(1).sub(RANKS.effect.rank[8]()));
      ...
      effDesc(eff) {
        return {
          ...
          eff:
            ...
            format(eff.eff) +
            " to Booster Power" +
            (eff.eff.gte(eff.ss) ? ` <span class='soft'>(softcapped${eff.eff.gte(1.8e5) ? (eff.eff.gte(5e15) && !player.ranks.pent.gte(15) ? "^3" : "^2") : ""})</span>` : ""),
        };
      },
      bonus() {
        let x = E(0);
  ...
  main: {
    ...
    1: {
      ...
      can(x) {
        return player.rp.points.gte(this[x].cost) && !player.mainUpg.rp.includes(x);
      },
      buy(x) {
        if (this.can(x)) {
      ...
      18: {
        ...
        cost: E("e1.5e128"),
        effect() {
          let x = Decimal.pow(10, tmp.matters.upg[12].eff.max(1).log10().pow(0.8));
    ...
    3: {
      ...
      13: {
        unl() {
          return (player.md.break.active && player.qu.rip.active) || hasElement(128);
        },
    ...
    4: {
      ...
      can(x) {
        return player.qu.rip.amt.gte(this[x].cost) && !player.mainUpg.br.includes(x);
      },
      buy(x) {
        if (this.can(x)) {
