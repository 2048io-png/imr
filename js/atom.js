const ATOM = {
  gain() {
    ...
    x = x.root(5);
    if (player.ranks.tier.gte(60)) x = x.mul(RANKS.effect.tier[60]());
    if (player.ranks.tier.gte(150)) x = x.mul(2);
  ...
  quarkGain() {
    ...
    let os = tmp.c16active
      ? E("ee6")
      : E("ee90")
          .pow(tmp.dark.abEff.ApQ_Overflow || 1)
          .pow(treeEff("ct13") ? tmp.chal.eff[15] : 1);
  ...
  atomic: {
    ...
    effect() {
      let x = player.atom.atomic
        .max(1)
        .log(hasElement(23) ? 1.5 : 1.75)
        .pow(getEnRewardEff(1));
    ...
    powerEffect: [
      ...
      (x) => {
        ...
        let b = hasUpgrade("atom", 18)
          ? Decimal.pow(1.1, player.rp.points.add(1).log10().add(10).log10().mul(x.add(1).log10().add(10).log10()).root(3).sub(1)).mul(player.mass.add(1).log10().add(10).log10())
          : hasElement(19)
          ? player.mass
              .max(1)
              .log10()
...
function updateAtomTemp() {
  ...
  tmp.atom.gamma_ray_cost = E(2)
    .pow(player.atom.gamma_ray.scaleEvery("gamma_ray", false, [1, 1, 1, fp]))
    .floor();
