function setupHTML() {
  ...
  for (let x = 0; x < RANKS.names.length; x++) {
    ...
    for (let y = 0; y < keys.length; y++) {
      table += `<span id="ranks_reward_${rn}_${y}"><b>${RANKS.fullNames[x]} ${keys[y]}:</b> ${RANKS.desc[rn][keys[y]]}${
        RANKS.effect[rn][keys[y]] ? ` Currently: <span id='ranks_eff_${rn}_${y}'></span>` : ""
      }</span><br>`;
    }
    table += `</div>`;
  }
  ...
  for (let x = 0; x < PRES_LEN; x++) {
    ...
    for (let y = 0; y < keys.length; y++) {
      table += `<span id="pres_reward_${x}_${y}"><b>${PRESTIGES.fullNames[x]} ${keys[y]}:</b> ${PRESTIGES.rewards[x][keys[y]]}${
        PRESTIGES.rewardEff[x][keys[y]] ? ` Currently: <span id='pres_eff_${x}_${y}'></span>` : ""
      }</span><br>`;
    }
    table += `</div>`;
  }
  ...
  for (let x in BEYOND_RANKS.rewards) {
    ...
    for (let y in BEYOND_RANKS.rewards[x]) {
      table += `<span id="br_reward_${x}_${y}"><b>${getRankTierName(x + 5)} ${format(y, 0)}:</b> ${BEYOND_RANKS.rewards[x][y]}${
        ee && BEYOND_RANKS.rewardEff[x][y] ? ` Currently: <span id='br_eff_${x}_${y}'></span>` : ""
      }</span><br>`;
    }
    table += "<br>";
  }
  ...
  for (let x = 0; x < SCALE_TYPE.length; x++) {
    ...
    for (let y = 0; y < key.length; y++) {
      table += `<div id="scaling_${x}_${key[y]}_div" style="margin-bottom: 10px;"><b>${NAME_FROM_RES[key[y]]}</b> (<span id="scaling_${x}_${key[y]}_power"></span>): Starts at <span id="scaling_${x}_${
        key[y]
      }_start"></span></div>`;
    }
    table += `</div>`;
  ...
  for (let x = 0; x < CONFIRMS.length; x++) {
    table += `<div style="width: 100px" id="confirm_div_${x}"><img src="images/${x == 1 ? "dm" : CONFIRMS[x]}.png"><br><button onclick="player.confirms.${CONFIRMS[x]} = !player.confirms.${
      CONFIRMS[x]
...
function updateTickspeedHTML() {
  ...
  if (unl) {
    ...
    tmp.el.tickspeed_step.setHTML(
      (teff.step.gte(10) ? format(teff.step) + "x" : format(teff.step.sub(1).mul(100)) + "%") + (teff.step.gte(teff.ss) && !hasUpgrade("rp", 16) ? " <span class='soft'>(softcapped)</span>" : "")
    );
  ...
  for (let x = 0; x < RANKS.names.length; x++) {
    ...
    if (player.ranks_reward == x) {
      ...
      for (let y = 0; y < keys.length; y++) {
        ...
        if (unl) if (tmp.el["ranks_eff_" + rn + "_" + y]) tmp.el["ranks_eff_" + rn + "_" + y].setHTML(RANKS.effDesc[rn][keys[y]](RANKS.effect[rn][keys[y]]()));
      }
    }
  }
}
