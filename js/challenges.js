  ...
  if (tmp.stab[3] == 0) {
    ...
    if (player.chal.choosed != 0) {
      ...
      tmp.el.chal_ch_title.setTxt(
        `[${player.chal.choosed}]${CHALS.getScaleName(player.chal.choosed)} ${chal.title} [${
          format(player.chal.comps[player.chal.choosed], 0) + (tmp.chal.max[player.chal.choosed].gte(EINF) ? "" : "/" + format(tmp.chal.max[player.chal.choosed], 0))
        } Completions]`
      );
...
const CHALS = {
  ...
  1: {
    ...
    reward: () =>
      hasBeyondRank(2, 20) ? `Supercritical Rank & All Fermions start later, Super Overpower scales weaker by completions.` : `Super Rank starts later, Super Tickspeed scales weaker by completions.`,
    max: E(100),
    inc: E(5),
    pow: E(1.3),
    start: E(1.5e58),
    ...
    effDesc(x) {
      return hasBeyondRank(2, 20)
        ? formatMult(x.scrank) + " later to Supercritical Rank & All Fermions starting, " + formatReduction(x.over) + " weaker to Super Overpower scaling"
        : "+" + format(x.rank, 0) + " later to Super Rank starting, " + format(E(1).sub(x.tick).mul(100)) + "% weaker to Super Tickspeed scaling";
    },
  },
  2: {
    unl() {
    ...
    effDesc(x) {
      return "+" + format(x.mul(100)) + "%" + (x.gte(0.3) ? " <span class='soft'>(softcapped)</span>" : "");
    },
  },
  3: {
    unl() {
    ...
    effDesc(x) {
      return "^" + format(x) + (x.gte(3) ? " <span class='soft'>(softcapped)</span>" : "");
    },
  },
  4: {
    unl() {
  ...
  5: {
    ...
    effDesc(x) {
      return hasCharger(3) ? formatReduction(x) + " weaker" : format(E(1).sub(x).mul(100)) + "% weaker" + (x.log(0.97).gte(5) ? " <span class='soft'>(softcapped)</span>" : "");
    },
  },
  6: {
    unl() {
      return player.chal.comps[5].gte(1) || player.supernova.times.gte(1) || quUnl();
    },
    ...
    effDesc(x) {
      return "+" + format(x) + "x" + (x.gte(0.5) ? " <span class='soft'>(softcapped)</span>" : "");
    },
  },
  7: {
    unl() {
      return player.chal.comps[6].gte(1) || player.supernova.times.gte(1) || quUnl();
    },
    ...
    reward: () =>
      hasPrestige(2, 25)
        ...
        : `Each completion increases challenges 1-4 cap by 2.<br><span class="yellow">On 16th completion, unlock Elements</span>`,
    max: E(50),
    inc: E(16),
    pow: E(1.25),
    start: E(1.5e76),
    ...
    effDesc(x) {
      return hasPrestige(2, 25) ? formatReduction(x) + " weaker" : "+" + format(x, 0);
    },
  },
  8: {
    unl() {
      return player.chal.comps[7].gte(1) || player.supernova.times.gte(1) || quUnl();
    },
    title: "White Hole",
    ...
    effDesc(x) {
      return "^" + format(x) + (x.gte(2.3) ? " <span class='soft'>(softcapped)</span>" : "");
    },
  },
  9: {
    unl() {
