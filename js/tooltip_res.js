const TOOLTIP_RES = {
  mass: {
    ...
    desc() {
      ...
      if (tmp.overflowBefore.mass.gte(tmp.overflow_start.mass[0])) h += `<br>(<b>+${formatMass(tmp.overflowBefore.mass)}</b> gained before <b>overflow</b>)`;
  ...
  rp: {
    ...
    desc() {
      let h = `<i>
            Reach over <b>${formatMass(1e25)}</b> of normal mass to reset previous features for gain Rage Powers.
            </i>`;
  ...
  bh: {
    ...
    desc() {
      ...
      if (tmp.overflowBefore.bh.gte(tmp.overflow_start.bh[0])) h += `<br>(<b>+${formatMass(tmp.overflowBefore.bh)}</b> gained before <b>overflow</b>)`;
