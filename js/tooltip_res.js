const TOOLTIP_RES = {
  mass: {
    ...
    desc() {
      ...
      if (tmp.overflowBefore.mass.gte(tmp.overflow_start.mass)) h += `<br>(<b>+${formatMass(tmp.overflowBefore.mass)}</b> gained before <b>overflow</b>)`;
  ...
  bh: {
    ...
    desc() {
      ...
      if (tmp.overflowBefore.bh.gte(tmp.overflow_start.bh)) h += `<br>(<b>+${formatMass(tmp.overflowBefore.bh)}</b> gained before <b>overflow</b>)`;
  ...
  quarks: {
    ...
    desc() {
      ...
      if (tmp.overflowBefore.quark.gte(tmp.overflow_start.quark)) h += `<br>(<b>+${format(tmp.overflowBefore.quark, 0)}</b> gained before <b>overflow</b>)`;
  ...
  md: {
    ...
    desc() {
      ...
      if (tmp.overflowBefore.dm.gte(tmp.overflow_start.dm)) h += `<br>(<b>+${formatMass(tmp.overflowBefore.dm)}</b> gained before <b>overflow</b>)`;
      ...
      h += `
            <br class='line'><i>
            ${
              player.md.active ? `Reach <b>${formatMass(tmp.md.mass_req)}</b> of normal mass to gain Relativistic Particles, or cancel dilation.` : "Dilate mass, then cancel."
            }<br><br>When dilating mass, will reset for atom. While mass is dilated: all pre-atom resources, atomic power gains get multiplier's exponent raised to 0.8.<br>
            </i>`;
