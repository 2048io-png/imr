const ELEMENTS = {
  map: [`x_________________xvxx___________xxxxxxvxx___________xxxxxxvxx_xxxxxxxxxxxxxxxxvxx_xxxxxxxxxxxxxxxxvxx1xxxxxxxxxxxxxxxxvxx2xxxxxxxxxxxxxxxxv_v__3xxxxxxxxxxxxxx__v__4xxxxxxxxxxxxxx__`],
  la: [null, "*", "**", "*", "**"],
  ...
  canBuy(x) {
    ...
    return (
      res.gte(u.cost) &&
      !hasElement(x) &&
      ((hasInfUpgrade(6) && x <= 218) || player.qu.rip.active || !BR_ELEM.includes(x)) &&
  ...
  upgs: [
    ...
    },
    {
      ...
      effDesc(x) {
        return hasPrestige(0, 867) ? "^" + format(x) : format(x) + "x" + softcapHTML(x, "ee4");
      },
    },
    {
    ...
    },
    {
      ...
      effect() {
        ...
        if (player.ranks.tier.gte(500)) x = x.mul(2);
        return x;
      },
      effDesc(x) {
        return "+" + format(x);
    ...
    },
    {
      ...
      effect() {
        ...
        if (player.ranks.rank.gte(12000)) x = x.mul(2);
    ...
    },
    {
      ...
      effDesc(x) {
        return format(x[0]) + "x" + (hasPrestige(0, 190) ? ", ^" + format(x[1]) : "");
      },
    },
    {
    ...
    },
    {
      ...
      effect() {
        ...
        if (player.ranks.rank.gte(24000)) x = player.supernova.times.mul(6);
        return x;
      },
      effDesc(x) {
    ...
    },
    {
      ...
      effect() {
        ...
        if (hasElement(79)) x = x.mul(tmp.qu.chroma_eff[2]);
        if (player.ranks.tier.gte(1000)) x = x.mul(2);
        return x;
      },
      effDesc(x) {
        return "+" + format(x, 0);
    ...
    },
    {
      ...
      cost: E("e710"),
      effect() {
        let x = player.dark.matters.final.pow(0.8).add(2).pow(player.dark.matters.final);
...
function isElemCorrupted(x, layer = 0) {
  return layer == 0 && !tmp.elements.deCorrupt.includes(x) && CORRUPTED_ELEMENTS.includes(x);
}

function hasElement(x, layer = 0) {
  return player.atom[["elements", "muonic_el"][layer]].includes(x) && !(tmp.c16active && isElemCorrupted(x));
}
      ...
      if (m == "v") table += `</div><div class="table_center">`;
        ...
        table +=
          ELEMENTS.upgs[num] === undefined
            ...
            : `<button class="elements ${num == 118 ? "final" : ""}" id="elementID_${num}" onclick="buyElement(${num}); ssf[0]('${
                ELEMENTS.names[num]
...
function updateElementsHTML() {
  ...
  if (ch) {
    ...
    tmp.el.elem_cost.setTxt(
      format(eu.cost, 0) + res + (eu.c16 ? " in Challenge 16" : BR_ELEM.includes(ch) ? " in Big Rip" : "") + (player.qu.rip.active && tElem.cannot.includes(ch) ? " [CANNOT AFFORD in Big Rip]" : "")
    );
            ...
            upg.setClasses(
              c16 && isElemCorrupted(x, elayer)
                ...
                : {
                    ...
                    muon: elayer == 1,
                    br: elayer == 0 && BR_ELEM.includes(x),
