...
const SCALING_RES = {
  ...
  massUpg(x = 1) {
    return E(player.massUpg[x] || 0);
  },
  bh_condenser(x = 0) {
    return player.bh.condenser;
  },
  gamma_ray(x = 0) {
    return player.atom.gamma_ray;
  },
  supernova(x = 0) {
  ...
  massUpg4() {
    return E(player.massUpg[4] || 0);
  },
  FSS() {
    return player.dark.matters.final;
  },
  pe() {
    return player.inf.pe;
  },
  inf_theorem() {
  ...
  for (let x = 0; x < SCALE_TYPE.length; x++) {
    ...
    for (let y = 0; y < key.length; y++) {
      ...
      if (noScalings(x, sn)) ns.push(sn);
        ...
        if (sn == "massUpg")
          for (let i = 0; i < UPGS.mass.cols; i++) {
...
function getScalingName(name, x = 0, y = 0) {
  ...
  for (let n = cap - 1; n >= 0; n--) {
    if (scalingActive(name, amt, Object.keys(SCALE_START)[n])) return capitalFirst(Object.keys(SCALE_START)[n]) + (n == 3 ? "-" : " ");
  }
  return current;
}
