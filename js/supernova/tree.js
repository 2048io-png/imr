...
const TREE_UPGS = {
  ...
  ids: {
    ...
    m1: {
      ...
      effDesc(x) {
        return hasElement(164) || hasElement(219) ? "^" + format(x) : format(x) + "x" + (x.max(1).log(1e100).gte(1e3) ? " <span class='soft'>(softcapped)</span>" : "");
      },
    },
    m2: {
      branch: ["m1"],
    ...
    ct12: {
      ...
      req() {
        return tmp.c16active && player.supernova.fermions.choosed == "06" && player.bh.mass.gte("1e1960") && player.bh.condenser.lte(0);
      },
      reqDesc() {
...
function updateTreeHTML() {
  ...
  if (tmp.supernova.tree_choosed != "")
    req = t_ch.req ? `<span class="${t_ch.req() ? "green" : "red"}">${t_ch.reqDesc ? " Requirement: " + (typeof t_ch.reqDesc == "function" ? t_ch.reqDesc() : t_ch.reqDesc) : ""}</span>` : "";
