...
const TREE_IDS = [
  ...
  [["s4", "sn5", "sn4"], ["", "", "", "qu_qol8a"], [], ["", "fn7", "fn8", "fn13"], ["qu6", "qu7", "qu8", "qu9", "qu10", "qu11"], []],
...
const TREE_UPGS = {
  ...
  ids: {
    ...
    m1: {
      ...
      cost: E(100),
      effect() {
        let x = hasElement(164) ? player.supernova.stars.add(1).log10().add(1).log10().div(10).add(1) : E(1e100).pow(player.supernova.stars.add(1).log10().pow(5).softcap(1e3, 0.25, 0));
        return x;
      },
      effDesc(x) {
        return hasElement(164) ? "^" + format(x) : format(x) + "x" + (x.max(1).log(1e100).gte(1e3) ? " <span class='soft'>(softcapped)</span>" : "");
      },
    },
    m2: {
      branch: ["m1"],
    ...
    rp1: {
      ...
      cost: E(200),
      effect() {
        let x = hasElement(165) ? player.supernova.stars.add(1).log10().add(1).log10().div(10).add(1) : E(1e50).pow(player.supernova.stars.add(1).log10().pow(5).softcap(1e3, 0.25, 0));
        return x;
      },
      effDesc(x) {
        return hasElement(165) ? "^" + format(x) : format(x) + "x" + (x.max(1).log(1e50).gte(1e3) ? " <span class='soft'>(softcapped)</span>" : "");
      },
    },
    bh1: {
      branch: ["c"],
      ...
      cost: E(400),
      effect() {
        let x = hasElement(166) ? player.supernova.stars.add(1).log10().add(1).log10().div(10).add(1) : E(1e35).pow(player.supernova.stars.add(1).log10().pow(5).softcap(1e3, 0.25, 0));
        return x;
      },
      effDesc(x) {
        return hasElement(166) ? "^" + format(x) : format(x) + "x" + (x.max(1).log(1e35).gte(1e3) ? " <span class='soft'>(softcapped)</span>" : "");
      },
    },
    bh2: {
      branch: ["bh1"],
    ...
    chal3: {
      ...
      reqDesc() {
        return `Reach ${format("e1.75e4")} uni of black hole without challenge 5-8 completions in a Supernova run.`;
      },
    ...
    fn2: {
      branch: ["fn1"],
      req() {
        return player.mass.div("1.5e56").gte("ee6") && player.md.active && FERMIONS.onActive("01");
      },
      reqDesc() {
        return `Reach ${formatMass(E("e1e6").mul(1.5e56))} while dilating mass in [Down]`;
      },
    ...
    fn3: {
      branch: ["fn1"],
      req() {
        return player.supernova.fermions.points[0].gte(1e7) || player.supernova.fermions.points[1].gte(1e7);
      },
      reqDesc() {
    ...
    fn6: {
      branch: ["fn2"],
      req() {
        return player.mass.gte(uni("e4e4")) && FERMIONS.onActive("02") && CHALS.inChal(5);
      },
      reqDesc() {
        return `Reach ${formatMass(uni("e4e4"))} while in [Charm] & Challenge 5.`;
      },
    ...
    fn10: {
      ...
      branch: ["fn5"],
      req() {
        return player.atom.points.gte("e1.5e8") && FERMIONS.onActive("10") && CHALS.inChal(9);
      },
      reqDesc() {
    ...
    qu_qol7: {
      ...
      reqDesc() {
        return `Reach ${formatMass(mlt(5e3))} of mass without completing Challenges 9-12 in Quantum run, while in [Bottom].`;
      },
    ...
    qc2: {
      ...
      reqDesc() {
        return `Reach ${formatMass(uni("ee5"))} of mass with QS 70 build (before bonus).`;
      },
    ...
    br1: {
      ...
      branch: ["qu5"],
      req() {
        return tmp.qu.qc_s >= 76 && player.mass.gte(uni("e7500")) && QCs.active();
      },
      reqDesc() {
        return `Reach ${formatMass(uni("e7500"))} of mass with 76 QS build (before bonus).`;
      },
    ...
    ct3: {
      ...
      req() {
        return tmp.c16active && player.supernova.fermions.choosed == "06" && player.bh.mass.gte("1e81");
      },
      reqDesc() {
    ...
    ct7: {
      ...
      req() {
        return player.chal.comps[14] && player.chal.comps[14].gte(777);
      },
      reqDesc() {
        return `Get ${format(777, 0)} C14 completions.`;
      },
    },
    ct8: {
      branch: ["ct2"],
    ...
    ct9: {
      ...
      req() {
        return tmp.c16active && player.supernova.fermions.choosed == "16" && player.bh.mass.gte("1e400") && player.bh.condenser.lte(0);
      },
      reqDesc() {
    ...
    ct12: {
      ...
      req() {
        return tmp.c16active && player.supernova.fermions.choosed == "06" && player.bh.mass.gte("1e2070") && player.bh.condenser.lte(0);
      },
      reqDesc() {
        return `Reach ${formatMass("1e2070")} of black hole during C16 & [Meta-Quark] without buying BH Condensers.`;
      },
    ...
    ct13: {
      ...
      req() {
        return player.chal.comps[14] && player.chal.comps[14].gte(960);
      },
      reqDesc() {
        return `Get ${format(960, 0)} C14 completions.`;
      },
    },
    ct14: {
      branch: ["ct11"],
      ...
      req() {
        return tmp.c16active && player.atom.atomic.gte(1e180);
      },
      reqDesc() {
        return `Reach ${format(1e180)} atomic powers during C16.`;
      },
...
function hasTree(id) {
  return (player.supernova.tree.includes(id) || player.dark.c16.tree.includes(id)) && !(tmp.c16active && CORRUPTED_TREE.includes(id));
}
      ...
      for (let x = 0; x < tmp.supernova.tree_had2[i].length; x++) {
        ...
        if (unl)
          tmp.el["treeUpg_" + id].setClasses(
            player.dark.c16.tree.includes(id) || (c16 && CORRUPTED_TREE.includes(id))
              ? { btn_tree: true, corrupted: true, choosed: id == tmp.supernova.tree_choosed }
              : { btn_tree: true, qu_tree: TREE_UPGS.ids[id].qf, locked: !tmp.supernova.tree_afford[id], bought: bought, choosed: id == tmp.supernova.tree_choosed }
          );
      }
  }
}
